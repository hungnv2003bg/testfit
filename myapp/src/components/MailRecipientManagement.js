...
export default function MailRecipientManagement() {
  ...
  const labels = {
    vi: {
      title: "Quản lý danh sách mail nhận thông báo SOPs",
      add: "Thêm mới",
      edit: "Chỉnh sửa",
      delete: "Xóa",
      email: "Email",
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`/api/mail-recipients/${id}`);
  ...
  const handleSubmit = async (values) => {
    try {
      if (editingRecipient) {
        await axios.put(`/api/mail-recipients/${editingRecipient.id}`, values);
      } else {
        await axios.post('/api/mail-recipients', values);
      }
      notification.success({
        message: 'Hệ thống',
        description: t.saved,
